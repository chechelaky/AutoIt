...
Func _BASS_FX_BPM_CallbackSet($handle, $BPMPROC, $period, $minMaxBPM, $flags, $user)
	Local $proc = DllCallbackRegister ($BPMPROC, "none", "dword;float;ptr")
	Local $_BASSFX_ret_ = DllCall ($_ghbassFXDll, "int", "BASS_FX_BPM_CallbackSet", "dword", $handle, "ptr", DllCallbackGetPtr ($proc), "double", $period, "dword", $minMaxBPM, "dword", $flags, "dword", $user)
	If @error Then Return SetError(1,1,0)
	If $_BASSFX_ret_[0] = $BASS_DWORD_ERR Then Return SetError(_BASS_ErrorGetCode(),0, 0)
	Return $_BASSFX_ret_[0]
EndFunc
...
Func _BASS_FX_BPM_Free($handle)
	DllCall ($_ghbassFXDll, "int", "BASS_FX_BPM_Free", "dword", $handle)
EndFunc
...
Func _BASS_FX_BPM_BeatCallbackSet($handle, $proc, $user)
	...
	Local $_BASSFX_ret_ = DllCall ($_ghbassFXDll, "int", "BASS_FX_BPM_BeatCallbackSet", "dword", $handle, "ptr", DllCallbackGetPtr ($proc), "ptr", $user)
	If @error Then Return SetError(1,1,0)
	If $_BASSFX_ret_[0] = $BASS_DWORD_ERR Then Return SetError(_BASS_ErrorGetCode(),0, 0)
	Return $_BASSFX_ret_[0]
EndFunc
...
Func _BASS_FX_BPM_BeatDecodeGet($chan, $startSec, $endSec, $flags, $proc, $user)
	$proc = DllCallbackRegister ($proc, "none", "dword;double;ptr")
	Local $_BASSFX_ret_ = DllCall ($_ghbassFXDll, "int", "BASS_FX_", "dword", $chan, "double", $startSec, "double", $endSec, "dword", $flags, "ptr", DllCallbackGetPtr ($proc), "ptr", $user)
	If @error Then Return SetError(1,1,0)
	If $_BASSFX_ret_[0] = $BASS_DWORD_ERR Then Return SetError(_BASS_ErrorGetCode(),0, 0)
	Return $_BASSFX_ret_[0]
EndFunc
