...
Global $tSound4 = Add_Sound(@ScriptDir & "\sound\crunch.wav")
...
Global $img5 = Image_Create("verde2.png", 300, 300, 8, 300, 1000, 2)
...
Func Image_Create($sPath, $xx = 0, $yy = 0, $iSpeed = 4, $iRange = 50, $iDelay = 100, $tSound = 0)
	Local $ds = DllStructCreate($tagWeapon)
	Local $id = _ArrayAdd($a_hImage, _GDIPlus_ImageLoadFromFile(@ScriptDir & "\images\" & $sPath))
	Local $ARR[22]
...
Func Image_Show()
	For $ii = 1 To $a_hImage[0]
		$iAngle = Radian2Degree(atan2($oUnity.Item(1).Item("x") -16.5- ($a_aiDim[$ii])[$POSX1], $oUnity.Item(1).Item("y") -16.5- ($a_aiDim[$ii])[$POSY1])) + 90
		...
		If Sqrt((($a_aiDim[$ii])[$POSX1] - $oUnity.Item(1).Item("x")-16.5) ^ 2 + (($a_aiDim[$ii])[$POSY1] - $oUnity.Item(1).Item("y")-16.5) ^ 2) <= ($a_aiDim[$ii])[$RANG2] Then
...
Func AcertouAlvo($xx, $yy)
	Local $limite = 33
	If $xx >= $oUnity.Item(1).Item("x")-16.5 And $xx <= ($oUnity.Item(1).Item("x") + $limite-16.5) And $yy >= $oUnity.Item(1).Item("y")-16.5 And $yy <= ($oUnity.Item(1).Item("y")-16.5 + $limite) Then
		_box($hBackbuffer, $oUnity.Item(1).Item("x")-16.5, $oUnity.Item(1).Item("y")-16.5, $limite + 1, $limite + 1)
...
;~ 				$oUnity.Item($each).Item("y") += $oUnity.Item($each).Item("vy")
			...
			Case 4
		EndSwitch
		_GDIPlus_GraphicsDrawImageRectRect($hBackbuffer, $oImage.Item($oUnity.Item($each).Item("i")), 0, 0, 33, 33, $oUnity.Item($each).Item("x")-16.5, $oUnity.Item($each).Item("y")-16.5, 33, 33)
	Next
EndFunc   ;==>Image_Unit
