...
Global $hGraphic, $hPen, $hPen2, $hBitmap, $hBackbuffer, $hCursor, $mouse[4], $xx2, $yy2
...
Global $iSeno = False, $iCos = False, $iTan = False, $iCot = False, $iSec = False, $iCoSec = False, $iInverso = False
...
While Sleep(10)
	...
	Switch $hMsg
		Case $GUI_EVENT_CLOSE
			Quit()
		Case $hSeno
			$iSeno = $iSeno == False ? True : False
		Case $hCos
			$iCos = $iCos == False ? True : False
		Case $hTan
			$iTan = $iTan == False ? True : False
		Case $hCot
			$iCot = $iCot == False ? True : False
		Case $hSec
			$iSec = $iSec == False ? True : False
		Case $hCosec
			$iCoSec = $iCoSec == False ? True : False
		Case $hInvertido
			$iInverso = $iInverso == False ? True : False
	EndSwitch
...
Func Update()
	...
	$tan = Int(200 * ($sen / $cos))
	$cot = Int(200 * ($cos / $sen))
	$sec = Int(200 * (1 / (Cos(($ang[0] / 180) * $PI))))
	$cosec = Int(200 * (1 / (Sin(($ang[0] / 180) * $PI))))
	...
	If $iSeno Then
		...
		_GDIPlus_GraphicsDrawLine($hBackbuffer, 389 + $cos, 270 - $sen, 389, 270 - $sen, $hPen)
	EndIf

	If $iCos Then
		_GDIPlus_GraphicsDrawLine($hBackbuffer, ($largura - 20) / 2 - 1, ($altura - 60) / 2, 389 + $cos, 270, $hPen2)
		_GDIPlus_GraphicsDrawLine($hBackbuffer, 389 + $cos, 270 - $sen, 389 + $cos, 270, $hPen)
	EndIf
	...
	If $iTan Then
		...
		If $cos > 0 Then
			...
			_GDIPlus_GraphicsDrawLine($hBackbuffer, 389 + $cos, 270 - $sen, 589, 270 - $tan, $hPen)
		Else
			_GDIPlus_GraphicsDrawLine($hBackbuffer, 189, 270 + $tan, 189, 270, $hPen2)
			_GDIPlus_GraphicsDrawLine($hBackbuffer, 389 + $cos, 270 - $sen, 189, 270 + $tan, $hPen)
		EndIf
	EndIf
	...
	If $iCot Then
		...
		If $sen > 0 Then
			...
			_GDIPlus_GraphicsDrawLine($hBackbuffer, 389 + $cos, 270 - $sen, ($largura - 20) / 2 - 1 + $cot, 70, $hPen)
		Else
			_GDIPlus_GraphicsDrawLine($hBackbuffer, ($largura - 20) / 2 - 1, 470, ($largura - 20) / 2 - 1 - $cot, 470, $hPen2)
			_GDIPlus_GraphicsDrawLine($hBackbuffer, 389 + $cos, 270 - $sen, ($largura - 20) / 2 - 1 - $cot, 470, $hPen)
		EndIf
	EndIf
